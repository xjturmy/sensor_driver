...
class State {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.path[i] = geometry_msgs.msg.PoseWithCovarianceStamped.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.path !== undefined) {
      ...
      for (let i = 0; i < resolved.path.length; ++i) {
        resolved.path[i] = geometry_msgs.msg.PoseWithCovarianceStamped.Resolve(msg.path[i]);
      }
    }
    else {
      resolved.path = []
