...
#include <perception_msgs/radarobjects.h>
...
ros::Publisher objects_pub_perception;
ros::Publisher objects_pub;
ros::Publisher detections_pub;
ros::Publisher status_pub;

std::string frame_name = "radar_front_link";


// caoxl add 新版本消息发布函数
void publishObjectListNew(RadarObjectList list)
{
    // ars548_msg::objects obj;
    // ars548_msg::ObjectList obj_list;

    
    perception_msgs::RadarObjectList obj_list;
    
    obj_list.header.frame_id = frame_name;
    ...
    // std::cout << "list.ObjectList_NumOfObjects---1----:"  <<(int)list.ObjectList_NumOfObjects<< std::endl;

    for(int i=0;i<list.ObjectList_NumOfObjects;i++)
    {
        
        // obj.header.frame_id = frame_name;
        // obj.header.stamp = ros::Time::now();
        perception_msgs::radarobjects obj;
        ...
        // ROS_INFO_STREAM("-----1------>"<<obj.u_Shape_Width_Edge_Mean);
       
        obj_list.object_array.push_back(obj);
    }

    objects_pub_perception.publish(obj_list);
...
void publishObjectList(RadarObjectList list)
{
    ...
    obj_list.ExtendedQualifier = list.ExtendedQualifier;
    obj_list.ObjectList_NumOfObjects = list.ObjectList_NumOfObjects;
    // std::cout << "list.ObjectList_NumOfObjects---2----:" <<std::hex<<list.ObjectList_NumOfObjects<< std::endl;
    ...
    for(i=0;i<list.ObjectList_NumOfObjects;i++)
    {
        ...
        // ROS_INFO_STREAM("-----2------>"<<obj.u_Shape_Width_Edge_Mean);
        obj_list.object_array.push_back(obj);
    }
...
void publishDetectionList(RadarDetectionList list)
{
    ...
    for(i=0;i<list.List_NumOfDetections;i++)
    {
        ...
        det.header.frame_id = frame_name;
...
void publishBasicStatus(RadarBasicStatus status)
{
    ...
    sts.header.frame_id = frame_name;
    ...
    switch(len)
    {
        case 9401:
            if(data_process.processObjectListMessage(data,&object_list))
            {
                ...
                {
                    // std::cout << "list.ObjectList_NumOfObjects---0----:" <<(int)object_list.ObjectList_NumOfObjects << std::endl;
                    publishObjectListNew(object_list);  //caoxl add
...
void receiveThread()
{
    ...
    {   
        ...
        if(num%10==1)
        {
            ROS_INFO_STREAM("--IP_addr--rec_length-->"<< rec_length <<", "<< source_address.sin_addr.s_addr);
        }
...
int main(int argc, char **argv)
{
    ...
    //caoxl add
    objects_pub_perception = nh.advertise<perception_msgs::RadarObjectList>("/perception/radardata", 10);  
    ...
    // udp_io.initUdpUnicastClient("10.13.1.115",42101,42401);
    udp_io.initUdpUnicastClient("192.168.8.207",42101,42401);
