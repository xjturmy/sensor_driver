...
#include <tf/transform_listener.h>  //add for transform
...
class OusterCloud : public nodelet::Nodelet {
   private:
    ...
    virtual void onInit() override {
        ...
        pub_transforms_ = nh.advertise<sensor_msgs::PointCloud2>("/perception/lidar_up_pointcloud", 10);
    ...
    void convert_scan_to_pointcloud_publish(std::chrono::nanoseconds scan_ts,
        ...
        for (int i = 0; i < n_returns; ++i) {
            ...
            stamp_transform_ = pc.header.stamp;
            //坐标转换接口
            try{
                listener.lookupTransform("base_link", "os_sensor", ros::Time(0), transform);
                }
            ...
            // std::cout << "输出的位置坐标：x=" << transform.getOrigin().getX() << ",y=" << transform.getOrigin().getY()
            //             << ",z=" << transform.getOrigin().getZ() << std::endl;

            pcl::PointCloud<pcl::PointXYZI>::Ptr combined_points_temp(new pcl::PointCloud<pcl::PointXYZI>);
            pcl::PointCloud<pcl::PointXYZI>::Ptr combined_points(new pcl::PointCloud<pcl::PointXYZI>);
            pcl::fromROSMsg(pc, *combined_points_temp);                   // ros转pcl点云
            pcl::transformPointCloud(*combined_points_temp, *combined_points, tf_btol);
            sensor_msgs::PointCloud2 combined_points_msg;                 // 发布预处理点云
            pcl::toROSMsg(*combined_points, combined_points_msg);  // 点云转换为msg
            combined_points_msg.header.frame_id = "base_link";
            combined_points_msg.header.stamp = stamp_transform_;
            pub_transforms_.publish(combined_points_msg);
